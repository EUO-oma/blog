...
export default function TodoPage() {
  ...
  useEffect(() => {
    ...
    const saved = window.localStorage.getItem('todoplan_sort_mode')
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return
    window.localStorage.setItem('todoplan_sort_mode', sortMode)
  }, [sortMode])
  ...
  if (!user) return <GuestPlaceholder title="Login to use TodoPlan" desc="Your tasks appear after sign-in." hint="U need login" buttonLabel="Login" emoji="☑️" />

  return (
    <main className="w-full max-w-none mx-0 px-0 space-y-1">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 pl-[10px]">
          <h1 className="text-2xl sm:text-3xl font-bold">Todo Plan</h1>
          <button
            onClick={async () => {
              await load()
              flashMsg('동기화 완료')
